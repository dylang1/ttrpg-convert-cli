...
public interface Pf2eTypeReader extends JsonSource {
    ...
    enum Pf2eDefenses implements JsonNodeReader {
        ...
        public static List<QuteDataHpHardness> getHpHardness(JsonNode source, Pf2eTypeReader convert) {
            ...
            Map<String, String> hpNotes = new HashMap<>();
            if (notes.isArrayIn(hpValueNode)) {
                hpNotes.put("hp", notes.joinAndReplace(hpValueNode, convert));
            } else {
                hpNotes.putAll(notes.getMapOfStrings(hpValueNode, convert.tui()));
            }
